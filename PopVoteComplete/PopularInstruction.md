...
*ซึ่งเกณฑ์คะแนนสามารถเพิ่มหรือลดได้ โดยจะต้องครอบ "["....."]" ไว้ เพราะตัวโค้ดจะดักจับจากตรงนี้
  ...
  **การกรองและจัดประเภทผลโหวต**
  ...
  - 'Unfaithful submit' : เกิดได้ 2 กรณีคือ
     ...
     - โหวตคะแนนให้กลุ่มที่ยังไม่ได้นำเสนอ (อัพเดตที่่ 'Add group')
...
## function การทำงาน
  1. **Start Script** ทำการ set up script ทั้งการรับข้อมูลนักเรียนทุกคน, เก็บข้อมูลใน cache, สร้าง sheet '_voted' (ถ้าไม่มี *ถ้ามีอยู่แล้วจะไม่แก้ไขข้อมูลข้างใน*), เก็บข้อมูลของกลุ่มที่นำเสนอแล้วที่ได้จาก function 'Add Group' ใน cache (*หมายเหตุ: cache นั้นจะเก็บข้อมูลได้ 4 ชั่วโมง ถ้าหมดเวลาหรือ refresh หน้าเว็บ ข้อมูลจะหายไป*), เพิ่ม column 'Error type' เข้าไปใน sheet ที่รับผลโหวต เพื่อแยกประเภทของผลโหวตที่รับเข้ามา มีไว้เพื่อใช้คำนวณคะแนนและเก็บสถิติต่อ
  2. **Add Group** เนื่องจาก
  3. **Announce winner**
    - me
